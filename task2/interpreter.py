...
class TokenType:
    ...
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"         # Оператор множення
    DIV = "DIV"         # Оператор поділу
...
class Parser:
    ...
    def factor(self):
        """Обробляємо числові значення та вирази в дужках."""
    ...
    def term(self):
        """Обробляємо вирази з операціями множення та поділу."""
        node = self.factor()
    ...
    def expr(self):
        """Обробляємо вирази з операціями додавання та віднімання."""
        node = self.term()
...
def run_tests():
    test_cases = [
        ...
        ("2 * 3", 6),
        ("6 / 3", 2),
        ("7 + 3 * 2", 13),   # Перевірка пріоритету операцій
        ("(2 + 3) * 4", 20),  # Перевірка роботи з дужками
        ("7 + (3 - 1) * 5", 17),
        ("10 - (2 + 3) * 2", 0),
        ("(10 - 3) * (2 + 2)", 28),
        ("((2 + 3) * 2) + 1", 11)
